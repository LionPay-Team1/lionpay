 ...
 * 환율 조회 API 부하 테스트
 *
 * 실행: k6 run wallet/exchange-rates.js
 * 스모크: k6 run -e SMOKE=true wallet/exchange-rates.js
 */
import { sleep } from 'k6';
import { loadOptions, smokeOptions } from '../config/options.js';
import { signUp, generateUser } from '../helpers/auth.js';
import { getExchangeRates } from '../helpers/wallet.js';
...
export function setup() {
    ...
    console.log(`Created ${users.length} test users for exchange-rates test`);
    return { users };
}
...
export default function (data) {
    ...
    getExchangeRates(user.accessToken);
