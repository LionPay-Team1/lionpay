# AdminSummaryModel


## Properties

Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**totalWallets** | [**AdminSummaryModelTotalWallets**](AdminSummaryModelTotalWallets.md) |  | [default to undefined]
**totalMerchants** | [**AdminSummaryModelTotalWallets**](AdminSummaryModelTotalWallets.md) |  | [default to undefined]
**totalTransactions** | [**AdminSummaryModelTotalWallets**](AdminSummaryModelTotalWallets.md) |  | [default to undefined]
**activeCurrencies** | [**V1WalletAdminExchangeRatesHistoryGetLimitParameter**](V1WalletAdminExchangeRatesHistoryGetLimitParameter.md) |  | [default to undefined]

## Example

```typescript
import { AdminSummaryModel } from './api';

const instance: AdminSummaryModel = {
    totalWallets,
    totalMerchants,
    totalTransactions,
    activeCurrencies,
};
```

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)
